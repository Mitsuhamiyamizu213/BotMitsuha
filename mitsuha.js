...
//
const BotName = 'MITSUHA BOT'; // Nama Bot Whatsapp
const instagramlu = 'https://www.instagram.com/adly.fahreza'; // Nama Instagramlu cok
const whatsapplu = '0888-8681-658'; // Nomor whatsapplu cok
const kapanbotaktif = '24 Jam'; // Kapan bot lu aktif
const grupch1 = 'https://chat.whatsapp.com/IaF1WLRZS1vLvSuki9ipR7'; // OFFICIAL GRUP LU 1
const grupch2 = 'https://chat.whatsapp.com/JHgUISoG2bYCQLVXiFANaf'; // OFFICIAL GRUP LU 2
//
const
{
   WAConnection,
...
conn.on('qr', qr =>
{
   ...
   console.log(`[ ${moment().format("HH:mm:ss")} ] MITSUHA CODE READY TO SCAN!`);
});
...
conn.on('user-presence-update', json => console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by MITSUHA`))
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by MITSUHA`)
})
...
if (text.includes("#gtworld")){
  const teks = text.replace(/#gtworld /, "")
conn.sendMessage(id, `Download Sendiri lah Bep :)\nntar server ku down, soalnya file gambar besar\n\nNih Link:\nhttps://s3.amazonaws.com/world.growtopiagame.com/${teks}.png\n\n\nkalo error berarti ada Huruf Besar atau Worldnya Belom di */renderworld* sama ownernya` , MessageType.text);
}
  
if (text.includes("#apakah")){
  const teks = text.replace(/#apakah /, "")
  var barqah = ["iya", "tentu", "ya jelas", "yoi", "nggak", "gak lahh", "tidak", "gk mungkin", "ya mana saya tau saya kan Bot"];
  var xiex = barqah[Math.floor(Math.random() * barqah.length)];
  conn.sendMessage(id, `pertanyaan: ${teks}\n\n\nJawab: ${xiex}` , MessageType.text);
}
  
if (text.includes("#sombong")){
  var barqah = ["Aduuuuuuuh, w alergi script hasil colongan", "Script Hasil Nyolong dari Youtube Jangan sok kerass👎🏻\naowkokwokwok", "script buatan yoga sakti ya\nngak bisa buat sendiri yak ?🤣", "Gw Hasil Codingan Barqah Xiex Boss -o-", "Bot kok hasil nyolong kok bangga waokwokawok", "Disaat Barqah Menciptakan Gw Di sana gw senang :)"];
  var xiex = barqah[Math.floor(Math.random() * barqah.length)];
  conn.sendMessage(id, xiex , MessageType.text);
}
  
if (text.includes("#gay")){
  const teks = text.replace(/#gay /, "")
  var xiex = Math.floor(Math.random() * 100);
  conn.sendMessage(id, `Ok Kita cek Seberapa Gay kah\n-${teks}-\n\nGay Meter:\n *${xiex}%* 🏳️‍🌈\nOMG!!!` , MessageType.text);
}
if (text.includes("#lesbi")){
  const teks = text.replace(/#lesbi /, "")
  var xiex = Math.floor(Math.random() * 100);
  conn.sendMessage(id, `Ok Kita cek Seberapa Lesbi kah\n-${teks}-\n\nLesbian Meter:\n *${xiex}%* 🏳‍⚧\nOMG!!!` , MessageType.text);
}
...
conn.sendMessage(id, 'kalo bingung ketik #help ya tod..' ,MessageType.text);
}
else if (text == 'woi'){
conn.sendMessage(id, 'Ya?, ada yang bisa saya bantu? kalo bingung ketik #help ya say..' ,MessageType.text);
}
...
conn.sendMessage(id, 'Bukan #stiker tapi #sticker :)' ,MessageType.text);
}
else if (text == '#donate'){
const corohelp = await get.get('https://covid19.mathdro.id/api/countries/id').json()
var date = new Date();
...
if (text.includes("#loli"))
  ...
  if (text.includes("#meme"))
   {
    var items = ["meme%20indo", "meme%20indonesia", "meme", "meme%20kocak"];
    var cewe = items[Math.floor(Math.random() * items.length)];
    var url = "https://api.fdci.se/rep.php?gambar=" + cewe;
